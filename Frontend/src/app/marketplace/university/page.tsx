...
// Mock data for products with university field
...
export default function UniversityMarketplacePage() {
  ...
  // Filter products by university and category
  const filteredProducts = UNIVERSITY_PRODUCTS
    .filter(product => selectedUniversity === 'all' || product.university === selectedUniversity)
    ...
    <div className="container py-8">
      ...
      <section className="mb-12 rounded-lg bg-muted p-6 text-center md:p-12">
        ...
        <div className="flex flex-col items-center justify-center gap-4 md:flex-row">
          ...
          <Button onClick={() => setSelectedUniversity('all')} variant="outline" size="sm">
            Clear Filter
          </Button>
        </div>
      </section>
      ...
      <Tabs defaultValue="All" className="mb-8">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold">
            {selectedUniversity ? `Products at ${selectedUniversity}` : 'All University Products'}
          </h2>
          <TabsList>
                ...
                <Link href={`${ROUTES.PRODUCT}/${product.id}`} key={product.id}>
                  <Card className="h-full overflow-hidden transition-all hover:shadow-md">
                    ...
                    <CardFooter className="flex flex-col items-start border-t p-4 pt-2">
                      ...
                      <p className="text-xs text-muted-foreground">University: {product.university}</p>
                    </CardFooter>
                  </Card>
                </Link>
              ))}
